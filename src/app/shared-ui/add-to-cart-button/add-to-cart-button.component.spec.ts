...
describe('AddToCartButtonComponent', () => {
  ...
  it('displays numberOnly visual message when no items are in the cart', async () => {
    const quantity = 0;
  ...
  it('displays numberOnly visual message when some items are in the cart', async () => {
    const quantity = 3;
    const numberOnly = true;
    await setup(quantity, numberOnly);
  ...
  describe("Keyboard Navigation", () => {
    it('displays default visual message when no items are in the cart and invokes correct methods', async () => {
      // default setup is quantity: 0, numberOnly: undefined
      const { user, mockCartService, productId } = await setup();
